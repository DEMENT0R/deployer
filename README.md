...
стенд). Заполнить их нужно руками на хосте — панель `.env` не редактирует. Пока они пусты, копия
рабочей не станет.
...
- Один remote на инстанс. `.env` целевого проекта панель показывает только на чтение и не переключает
  под ветку/окружение; единственная запись в `.env` — разовая, при создании копии инстанса.
